function ard_make_rtw_hook(hookMethod,modelName,rtwroot,templateMakefile,buildOpts,buildArgs)
...
% Copyright 1996-2010 The MathWorks, Inc.
  ...
  switch hookMethod
   case 'error'
    ...
    disp('# There appears to be an error!');
   ...
   case 'entry'
    ...
    disp('# Hook File: "ard_make_rtw_hook.m"');
    disp('# Target File: "ard.tlc"');
   ...
   case 'before_tlc'
    ...
    % buildArgs
    disp('# before_tlc called');
    
   case 'after_tlc'
    % Called just after to invoking TLC Compiler (actual code generation.)
    % Valid arguments at this stage are hookMethod, modelName, and
    % buildArgs
    disp('# after_tlc called');

   case 'before_make'
    % Called after code generation is complete, and just prior to kicking
    % off make process (assuming code generation only is not selected.)  All
    % arguments are valid at this stage.
    disp('# before_make called');
    disp(['# Makefile: ' templateMakefile]);
    which('simple_noexternal.mk')
    which('arm-linux-gnueabihf-g++.exe')

   case 'after_make'
    % Called after make process is complete. All arguments are valid at 
    % this stage.
    disp('# after_make called');
